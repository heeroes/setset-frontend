<script setup>
import { ref, computed, defineProps, toRef, watch, watchEffect } from "vue";
import axios from "axios";

const alarms = ref(new Map());
const selectedAlarm = ref(null); // 선택된 알람을 저장하는 ref
const currentPage = ref(1);
const itemsPerPage = 5;
const pagesPerGroup = 5;
const props = defineProps(["planRegionList"]);
const planRegionList = toRef(props, "planRegionList");

const selectedRegion = ref("");

console.log("props selectedRegion : ", selectedRegion);

const getDisasterAlarm = async (region) => {
  console.log("get region ", region);
  const url = `https://setset.site/api/v1/safe/info/disaster/search?region=${region}&size=1000`;
  const { data } = await axios.get(url);
  console.log("response : ", data.result);
  alarms.value.set(region, data.result.alarms);
  console.log("alarm : ", alarms.value.get(selectedRegion.value));
};
watchEffect(() => {
  if (planRegionList.value && planRegionList.value.length > 0) {
    selectedRegion.value = planRegionList.value[0];

    for (let region of planRegionList.value) {
      console.log("for region : ", region);
      getDisasterAlarm(region);
    }
  }
});
const paginatedAlarms = computed(() => {
  ...
  const selectedRegionAlarms = alarms.value.get(selectedRegion.value);
  console.log("paginated select region : ", selectedRegionAlarms);
  return selectedRegionAlarms ? selectedRegionAlarms.slice(start, end) : [];
});

const totalPageCount = computed(() => {
  const selectedRegionAlarms = alarms.value.get(selectedRegion.value);
  return selectedRegionAlarms
    ? Math.ceil(selectedRegionAlarms.length / itemsPerPage)
    : 0;
});
  ...
  <div>
    <div>
      ...
      <select v-model="selectedRegion">
        <option v-for="region in planRegionList" :key="region" :value="region">
          {{ region }}
        </option>
      </select>
    </div>
